---
name: support-type
description: 为 Pioneer 新增文件类型支持时使用。必须同时在 core / react / vue 三个包中完成类型声明、类型识别、渲染器实现与接入，保证 React 和 Vue 两个框架同步支持。当用户提到"支持 xxx 类型"、"新增文件类型"、"添加 xxx 预览"时使用。
---
...
- `packages/core/` — 共享类型、工具、文件识别
- `packages/react/` — React 实现
- `packages/vue/` — Vue 实现
...
- [ ] `packages/core/src/utils/fileType.ts`
  - 在 `getFileType()` 中添加 mime / 扩展名的识别分支
  - 如果是媒体类，同步更新 `getVideoMimeType` / 语言表等
- [ ] 如需新增解析工具，放在 `packages/core/src/utils/` 下，框架无关
- [ ] **i18n 字典**：`packages/core/src/i18n/messages/zh-CN.ts` 与 `en-US.ts` 必须**同时**补齐新 renderer 用到的所有文案 key（详见 §3.7）
...
- [ ] `packages/react/src/renderers/Xxx/index.tsx` — 新增 React 渲染器
  ...
  - **主题适配**：颜色只能用语义 token 类（`pio-text-fg-*` / `pio-bg-surface-*` / `pio-border-line-*` / `pio-bg-code-bg` / `pio-bg-media-bg` 等），禁止 `text-white/XX`、`text-gray-N00`、`bg-[#1e1e1e]` 等字面色类（详见 §3.8）
  - 如果需要工具栏控制（翻页/缩放/全屏/目录等），必须用 `forwardRef` + `useImperativeHandle` 暴露 `XxxRendererHandle`
- [ ] `packages/react/src/renderers/Xxx/toolbar.tsx` — **如需工具栏，必加** 伴生 toolbar 配置（详见 §3.5）
- [ ] `packages/react/src/renderers/lazy.tsx` — **必加** lazy 注册项（详见 §3.6）
...
- [ ] `packages/react/src/PioneerContent.tsx`
...
- [ ] `packages/react/package.json` — 新增必要依赖
  - **重型 npm 依赖（解析库、UI 库、媒体库等）必须放在 `dependencies`**，**禁止**放在 `devDependencies`（否则消费者装包后会缺包；详见 §3.9）
  - 同时在 `packages/react/vite.config.ts` 的 `rollupOptions.external` 中加入对应条目（详见 §3.9）

### 3. Vue 包

- [ ] `packages/vue/src/renderers/Xxx/index.vue` — 新增 Vue 渲染器
  ...
  - 样式类名必须使用 `pio-` 前缀
  - **禁止中文/英文文案硬编码**，用 `const { t } = useTranslator()` + `t('xxx.key')`（详见 §3.7）
  - **主题适配**：颜色只能用语义 token 类（`pio-text-fg-*` / `pio-bg-surface-*` / `pio-border-line-*` 等），`<style scoped>` 中改用 `var(--pio-*)` 变量；禁止字面色类与硬编码 `rgba(255, 255, 255, ...)` / `#XXXXXX` 字面量（详见 §3.8）
  - 如果需要工具栏控制，必须用 `defineExpose({...})` 暴露与 React 同名的方法
- [ ] `packages/vue/src/renderers/Xxx/toolbar.ts` — **如需工具栏，必加** 伴生 toolbar 配置（详见 §3.5）
- [ ] `packages/vue/src/renderers/lazy.ts` — **必加** lazy 注册项（详见 §3.6）
  - 追加 `export const XxxRenderer = wrap(() => import('./Xxx/index.vue'));`
- [ ] `packages/vue/src/PioneerContent.vue`
...
- [ ] `packages/vue/package.json` — 新增必要依赖
  - **重型 npm 依赖必须放在 `dependencies`**，**禁止**放在 `devDependencies`（详见 §3.9）
  - 同时在 `packages/vue/vite.config.ts` 的 `rollupOptions.external` 中加入对应条目（详见 §3.9）

### 3.5 工具栏配置规范（数据驱动，沉到 renderer 伴生文件）

项目采用**数据驱动 + 伴生 toolbar 配置**的模式（commit `acb2e09` 重构后）。PioneerContent 不再通过 `showZoomControls` 这类布尔变量切换按钮，而是统一调 `getXxxToolbarGroups(ctx)` 拿到 `ToolbarGroup[]` 数组渲染。
...
export function getXxxToolbarGroups(ctx: XxxToolbarContext): ToolbarGroup[] {
  return [
    { items: [{ type: 'button', icon: <List className="pio-w-4 pio-h-4" />, tooltip: '目录', action: () => ctx.xxxRef.current?.toggleToc() }] },
    {
      items: [
        { type: 'button', icon: <ChevronLeft className="pio-w-4 pio-h-4" />, tooltip: '上一页', action: () => ctx.xxxRef.current?.prevPage() },
        { type: 'text', content: `${ctx.current} / ${ctx.total}`, minWidth: '4rem' },
        { type: 'button', icon: <ChevronRight className="pio-w-4 pio-h-4" />, tooltip: '下一页', action: () => ctx.xxxRef.current?.nextPage() },
      ],
    },
  ];
}
...
export interface XxxToolbarContext {
  // Vue 直接传 ref 解包后的对象（在 PioneerContent 里传 xxxRef.value）
...
> - **React**：`icon` 是 JSX 元素（`<List className="pio-w-4 pio-h-4" />`），`xxxRef` 是 `React.RefObject<Handle | null>`，调用时 `ctx.xxxRef.current?.method()`
> - **Vue**：`icon` 是组件本身（`List`，由模板侧用 `<component :is="icon" class="pio-w-4 pio-h-4" />` 渲染），`xxxRef` 是已解包的对象，调用时 `ctx.xxxRef?.method()`

#### PioneerContent 接入"五件事"
...
- ❌ **禁止**在 `PioneerContent` 内联硬编码工具栏按钮 — 必须沉到伴生 `toolbar.tsx|.ts`
...
为了让发布包主入口保持极小（React 主入口 gzip ≤ 80 KB / Vue ≤ 60 KB），所有 renderer **强制走 `React.lazy` / `defineAsyncComponent` 异步加载**，由 rollup 自动拆 chunk。每个新 renderer 必须同步在两个框架的 `renderers/lazy.{tsx,ts}` 中注册，**禁止**在 `PioneerContent` 中静态 import 任何 renderer 本体（toolbar 配置文件除外，它本来就在主入口）。

> ⚠️ 例外：`UnsupportedRenderer` 因为是所有错误回退的兜底，且自身体量极小，**保留**静态 import 直接打进主入口。除此之外**没有任何其它例外**。

#### React (`packages/react/src/renderers/lazy.tsx`)
...
#### Vue (`packages/vue/src/renderers/lazy.ts`)
...
`PioneerContent.vue` `<script setup>` 顶部：
...
如果 `pnpm size` 失败，几乎一定是把新 renderer 静态 import 进了主入口或 PioneerContent，回头检查 lazy.tsx/lazy.ts 是否补完。

#### 硬性禁止（懒加载相关）

- ❌ **禁止**在 `PioneerContent.{tsx,vue}` 中直接 `import { XxxRenderer } from './renderers/Xxx'`（除 `UnsupportedRenderer` 外）— 必须走 `lazy.{tsx,ts}` 间接导入
...
- ❌ **禁止**在 renderer 文件中再加 `<Suspense>` — 外层 `PioneerContent` 已统一包裹
...
唯一权威源：`packages/core/src/i18n/messages/zh-CN.ts` 与 `en-US.ts`。两个 framework 包都从 core 的 `builtInMessages` import，**严禁**在 react/vue 包本地再建字典。
...
- [ ] `PioneerContent.tsx|.vue` 调 `getXxxToolbarGroups({ ..., t })`（Vue 侧传 `t: t.value`）
...
组件支持 `theme: 'auto' | 'dark' | 'light'`，最终解析后通过 `data-theme="dark|light"` 挂在根容器 `.pio-root` / `.pio-root` 上。**项目用 CSS 变量定义语义化颜色 token，由 Tailwind 的 `theme.extend.colors` 暴露成语义类（`text-fg-primary`、`bg-surface-1`、`border-line` 等）**。Renderer 里只用语义类，主题切换只换 `[data-theme]` 上的变量值，class 完全不变。

> ⚠️ 历史风格（`text-white/XX`、`text-gray-700` 等字面色类）已**全部废弃**，禁止再使用。所有 renderer 的颜色必须落在下方 token 表中。

#### 设计 token 总表（**只能用这些**）

token 在 `packages/{react,vue}/src/index.css` 顶部定义为 `--pio-*` CSS 变量；Tailwind 配置在 `tailwind.config.js` 的 `theme.extend.colors` 中映射为类名。前缀：React 用 `pio-`，Vue 用 `pio-`。
...
```tsx
// React renderer
<div className="pio-text-fg-primary pio-bg-surface-1 pio-border pio-border-line-weak">
  <span className="pio-text-fg-tertiary">{description}</span>
  <button className="pio-text-fg-secondary hover:pio-bg-surface-2">{label}</button>
  <pre className="pio-bg-code-bg pio-text-code-fg">{code}</pre>
</div>
```
...
<!-- Vue renderer -->
<template>
  <div class="pio-text-fg-primary pio-bg-surface-1 pio-border pio-border-line-weak">
    <span class="pio-text-fg-tertiary">{{ description }}</span>
    <button class="pio-text-fg-secondary hover:pio-bg-surface-2">{{ label }}</button>
    <pre class="pio-bg-code-bg pio-text-code-fg">{{ code }}</pre>
  </div>
</template>

<style scoped>
/* scoped style 用 var(--pio-*) */
.my-block {
...
```tsx
// ❌ 字面色类已全部废弃
<div className="pio-text-white/90 pio-bg-white/10 pio-border-white/15">
<div className="pio-text-gray-700 pio-bg-gray-100 pio-border-gray-200">

// ❌ 内联绝对色（除非走 var(--pio-*)）
<div style={{ color: '#ffffff', background: '#1f2937' }}>
<div style={{ background: '#1e1e1e' }}>  {/* 应改用 className="pio-bg-code-bg" */}
...
<style scoped>
.foo { color: rgba(255, 255, 255, 0.75); }   /* 应该用 var(--pio-fg-secondary) */
.foo { background: #1e1e1e; }                 /* 应该用 var(--pio-code-bg) */
</style>
```
...
1. 在 `packages/react/src/index.css` 与 `packages/vue/src/index.css` 的 `.pio-root` / `.pio-root` 默认块加 `--pio-accent-warning: ...`
2. 在两个 css 的 `[data-theme="light"]` 覆盖块加 light 值（如果需要主题切换）
3. 在两个 `tailwind.config.js` 的 `theme.extend.colors` 加 `'accent-warning': 'var(--pio-accent-warning)'`
4. **重新 build：** `cd packages/react && pnpm build && cd ../vue && pnpm build`（example 引用 `dist/index.css`，不重新构建新类不会出现）
...
- ❌ **禁止**在 renderer 内联 `style={{ background: '#1e1e1e' }}` 或 `rgba(255,255,255,X)` 等硬编码颜色 — 用 `bg-code-bg` 或 `var(--pio-*)`
- ❌ **禁止**在 Vue `<style scoped>` 中写死 `rgba(255, 255, 255, X)` / `#XXXXXX` 等暗色字面量 — 全部用 `var(--pio-*)`
...
**核心约束（零 BREAKING 策略）**：外部化的依赖**必须同时**声明在 `packages/{react,vue}/package.json#dependencies` 中，npm/pnpm install 时自动拉取，使用者无需手动安装任何新包。
...
2. **加到 `rollupOptions.external`**（两个 vite.config.ts 都要）：
   ...
   ```ts
   // packages/{react,vue}/vite.config.ts
   external: [
     // ... 已有项
     'xxx-parser',
...
3. **跑 build 验证外部化**：
   ...
   grep -c 'from "xxx-parser"' packages/react/lib/chunks/*.mjs   # 有匹配
   grep -c 'PARSER_INTERNAL_FUNC' packages/react/lib/chunks/*.mjs # 应为 0
   ```
...
| 产物 | gzip 上限 |
|---|---|
| `packages/react/lib/index.mjs` | 80 KB |
| `packages/react/lib/**/*.mjs`（全量） | 800 KB |
| `packages/react/lib/**/*.css` | 400 KB |
| `packages/vue/lib/index.mjs` | 60 KB |
| `packages/vue/lib/**/*.mjs`（全量） | 500 KB |
| `packages/vue/lib/**/*.css` | 400 KB |
...
- ❌ **禁止**把 `@pioneer/core` 加进 `external` — 见底部硬性禁止第 12 条
...
- [ ] `packages/react/README.md` — 「Supported File Formats」章节 **+** 「Supported MIME Types」章节
- [ ] `packages/react/README.zh-CN.md` — 「支持的文件格式」章节 **+** 「支持的 MIME 类型」章节
- [ ] `packages/vue/README.md` — 「Supported File Formats」章节
- [ ] `packages/vue/README.zh-CN.md` — 「支持的文件格式」章节
...
grep -rn "'xxx'" packages/core/src packages/docs/api/types.md packages/docs/guide/supported-types.md
grep -rn "XXX\|\.xxx" packages/react/README.md packages/vue/README.md
```
...
```
packages/react/src/renderers/*Renderer.tsx
packages/vue/src/renderers/*Renderer.vue
```
...
2. React renderer + **lazy.tsx 注册** + 接入 PioneerContent + toolbar 接 `t`
3. Vue renderer + **lazy.ts 注册** + 接入 PioneerContent + toolbar 接 `t`
...
- 支持的交互（缩放 / 分页 / 下载）是否一致
- 错误状态 / 加载状态的处理是否一致
- 两边 `PioneerContent` 的 `fileType === 'xxx'` 分支是否都已接入
- **主题切换验证**：在 example 中切到 `Light` 主题打开新 renderer，肉眼确认无白底白字、无对比度不足，必要时同时切 `Dark` 回归（详见 §3.8 自检清单）

使用 `Grep` 命令交叉检查：

```
Grep: "'xxx'" in packages/core/src
Grep: "XxxRenderer" in packages/react/src
Grep: "XxxRenderer" in packages/vue/src
```
...
- ❌ **禁止**把与框架无关的解析逻辑写进 react/vue 包里——必须沉到 `core`
- ❌ **禁止**让 React 和 Vue 的 renderer 行为出现差异（交互、props、事件语义必须对齐）
- ❌ **禁止**跳过 `PioneerContent` 的接入步骤（新增 renderer 不接入等于没做）
...
- ❌ **禁止**在 renderer 里使用字面色类（`text-white/XX`、`text-gray-N00`、`bg-[#1e1e1e]`、`bg-white/XX`、`border-gray-N00` 等）或硬编码 `rgba(255,255,255,X)` / `#XXXXXX` — 全部用 §3.8 中的语义 token 类（`text-fg-*` / `bg-surface-*` / `border-line-*` / `bg-code-bg` 等）或 `var(--pio-*)` 变量
- ❌ **禁止**给 renderer 加 `theme?: Theme` props 或在 renderer 内部读 `data-theme` 切样式分支 — 主题适配统一交给 CSS 变量（详见 §3.8）
- ❌ **禁止**在 `PioneerContent.{tsx,vue}` 中静态 `import` 任何 renderer 本体（仅 `UnsupportedRenderer` 例外） — 必须经由 `renderers/lazy.{tsx,ts}` 间接导入，否则破坏代码分割（详见 §3.6）
...
- ❌ **禁止**把 `@pioneer/core` 加进 react / vue 两个发布包的 `vite.config.ts` 的 `rollupOptions.external` 数组 — core 是内部共享层，**永不发布到 npm**，必须内联打包进 react / vue 产物。若 external 列表里出现 core，消费者项目装包后会报 `Could not resolve "@pioneer/core"`。两个 vite.config.ts 里已经有警示注释「注意：@pioneer/core 未发布到 npm，必须内联打包」，不要删除
