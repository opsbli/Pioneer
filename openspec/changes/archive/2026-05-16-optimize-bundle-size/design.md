...
|---|---|---|---|---|
| `@pioneer/react` | **11.88 MB** | **2.27 MB** | 1.63 MB | 978 KB |
| `@pioneer/vue` | 2.26 MB | 521 KB | 1.60 MB | 975 KB |

经分析，问题根因有四条：

1. **强制单 chunk 内联**：[packages/react/vite.config.ts](packages/react/vite.config.ts) 与 [packages/vue/vite.config.ts](packages/vue/vite.config.ts) 均设置了 `inlineDynamicImports: true`，把 `foliate-js`、`pdfjs-dist`、`pptx-preview`、`shiki`、`docx-preview`、`mammoth`、`video.js` 等全部内联。
2. **静态全量注册**：[PioneerContent.tsx](packages/react/src/PioneerContent.tsx) 与 [PioneerContent.vue](packages/vue/src/PioneerContent.vue) 在文件顶部静态 import 全部 18 个 renderer，rollup 无法 tree-shake。
...
- 必须保持主入口 (`PioneerModal` / `PioneerEmbed` / `PioneerContent` / `normalizeFile` 等) 的 JS API 向后兼容（仅运行时行为可变）。
- 必须同步覆盖 React 与 Vue 两个包（依据 `support-type` skill 的约定）。
- 不引入新业务逻辑，仅做构建与依赖治理。
- 不能破坏 `@pioneer/core` 的内联策略（它必须保持内联打包，因为未发布 npm）。
...
- 不重写 `@pioneer/core`，仅在必要时调整其导出粒度。
- 不优化 `react-docs` 和示例工程的体积。

## Decisions

### D1: 采用"主入口壳 + 动态 renderer chunk"双层结构（而非纯多入口）

**选择**：保持单一主入口 `import { PioneerModal } from '@pioneer/react'`，在 `PioneerContent` 内部用 `React.lazy` / `defineAsyncComponent` 按 `fileType` 动态 import renderer。**同时**额外暴露子路径入口 `./renderers/<type>` 给高级用户。
...
- ❌ 纯多入口（强制使用者 `import {PioneerModal} from '@pioneer/react/modal'`）：BREAKING 太大，社区接受度低。
...
- 把 `index.css` 拆为：
  - `style/core.css`：pio-root 变量声明 + base + Modal/Embed/Content 三个壳组件样式。
  - `style/<type>.css`：每个 renderer 单独的 Tailwind 入口（仅 import 该 renderer 用到的 utility）。
- 修剪 [tailwind.config.js](packages/react/tailwind.config.js) 的 `content` 路径与 `safelist`，移除已不存在的 class。
...
- 是否需要把 `@pioneer/core` 单独发布到 npm，从 react/vue 包中 external 掉？（短期否，长期建议）
- 是否提供 `react/lite` 入口（只含 image/video/audio/markdown/text），开箱即用最小集？（待 beta 反馈定）
