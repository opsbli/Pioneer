...
- `@pioneer/react`：gzip 总和 ≤ 800 KB
- `@pioneer/vue`：gzip 总和 ≤ 500 KB
...
- **WHEN** 使用者执行 `import { PdfRenderer } from '@pioneer/react/renderers/pdf'`
- **THEN** TypeScript 类型检查通过
- **AND** 运行时正确返回 PdfRenderer 组件

#### Scenario: 主入口与子路径入口共享同一实现

- **WHEN** 同一页面同时使用主入口的 `PioneerModal` 与子入口 `PdfRenderer`
...
- `lib/style/core.css`：主入口必需样式（`.pio-root` 变量、Modal / Embed / Content 壳组件样式）
...
- **WHEN** 用户代码中保留 `import '@pioneer/react/style.css'`
