...
当前发布到 npm 的 `@pioneer/react` 产物 `lib/index.mjs` 高达 **11.88 MB**（gzip 2.27 MB），`lib/index.css` 高达 **1.6 MB**（gzip 978 KB）；`@pioneer/vue` 产物 2.25 MB（gzip 521 KB），CSS 同样高达 1.6 MB。体积严重超过同类预览库（一般 < 500 KB gzip），主要原因：
...
- **入口拆包**：在 `package.json#exports` 中暴露子路径入口，按 renderer 类型与场景额外暴露子入口：
  ...
  - 主入口 `.` 的所有原导出保持不变，向后兼容。
- **动态加载（默认行为）**：`PioneerContent` 内部按 `fileType` 动态 `import()` 对应 renderer 与其重依赖（pdfjs-dist、foliate-js、pptx-preview、docx-preview、mammoth、video.js、shiki、katex 等），关闭 ESM 输出的 `inlineDynamicImports`，让 rollup 自然 code-splitting。运行时按需异步下载对应 chunk；现代打包器（Vite / webpack 5 / esbuild / rspack）自动处理。
...
- **代码影响**：
  - `packages/react/vite.config.ts`、`packages/vue/vite.config.ts`：ESM 输出移除 `inlineDynamicImports`，开启 `cssCodeSplit`，扩展 `external`，新增多入口配置。
  - `packages/react/src/index.ts`、`packages/vue/src/index.ts`：在 `package.json#exports` 暴露子路径入口，但主入口导出 100% 不变。
  - `packages/*/src/PioneerContent.*`：renderer 注册由静态 import 改为 `React.lazy` / `defineAsyncComponent`，并提供 `loading` 占位。
...
- **API 影响**：
  - 主入口导出完全兼容（`PioneerModal`/`Embed`/`Content`、`normalizeFile` 等）。
  - 新增子入口属于增量，不破坏现有用法。
