...
- **架构变更**：
  - 新增 `packages/core/src/loaders/` 目录，每种格式一个独立 loader（`heicLoader.ts`、`tiffLoader.ts` 等）
...
- **MIME 类型扩展**：在 `packages/core/src/utils/mimeTypes.ts` 中新增扩展名到 MIME 映射（`image/heic`、`image/avif`、`image/tiff` 等）
- **UI 增强**：
...
- **代码影响**：
  - `packages/core/src/utils/mimeTypes.ts`：新增 10+ 种 MIME 类型映射
  - `packages/core/src/loaders/`：新增 6 个 loader 文件（heicLoader.ts、avifLoader.ts、tiffLoader.ts、rawLoader.ts、psdLoader.ts、jp2Loader.ts）
  - `packages/core/src/renderers/ImageRenderer.tsx`（React）/ `.vue`（Vue）：改造为先检测格式 → 需要解码则调用 loader → 生成 Blob URL 喂给 `<img>`
  - `packages/core/src/workers/imageDecoder.worker.ts`：新增 Web Worker 处理耗时解码
...
- **依赖影响**：
  ...
  - 不引入新 peer 依赖
- **API 影响**：
  - 主入口 API 完全兼容（`PioneerModal`/`Embed`/`Content` props 不变）
