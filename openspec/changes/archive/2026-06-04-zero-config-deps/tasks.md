...
- [x] 1.2 ~~执行 `pnpm --filter @pioneer/react-example dev` 与 `pnpm --filter @pioneer/vue-example dev`,在两端各预览一次 `.msg` / `.woff2` / `.jp2` 文件,确认基线行为正常~~ — 用户决定跳过

## 2. Step 1 — stream alias 单点落地

- [x] 2.1 新建 `packages/react/src/shims/stream-stub.ts`,内容与 `packages/example/src/shims/stream-stub.ts` 一致
- [x] 2.2 新建 `packages/vue/src/shims/stream-stub.ts`,内容与 `packages/vue-example/src/shims/stream-stub.ts` 一致
- [x] 2.3 在 `packages/react/vite.config.ts` 添加 `resolve.alias`,把 `stream` 指向 `src/shims/stream-stub.ts`
- [x] 2.4 在 `packages/vue/vite.config.ts` 添加 `resolve.alias`,把 `stream` 指向 `src/shims/stream-stub.ts`
- [x] 2.5 执行 `pnpm --filter @pioneer/react build` 与 `pnpm --filter @pioneer/vue build`,确认构建无 `stream externalized` 警告 — 两包均构建成功无警告;注意 Step 1 单独时 msgreader 仍 external,真正验证 alias 生效在 Step 2 完成后
...
- [x] 3.3 在 `packages/react/vite.config.ts` 把 `@kenjiuno/msgreader` 从 ESM external 列表中移除(CJS external 保留)
- [x] 3.4 在 `packages/vue/vite.config.ts` 同步移除
...
- [x] 4.1 在 `packages/react/vite.config.ts` 把 `wawoff2`(含正则)与 `opentype.js` 从 ESM external 移除(CJS external 保留)
- [x] 4.2 在 `packages/vue/vite.config.ts` 同步移除
...
- [x] 5.1 在 step 3 完成后的 `lib/` 中 grep `jpeg2000` 与 `@cornerstonejs/codec-openjpeg`,确认无任何引用(`core` 已通过 `?worker&inline` 全部内联到 jp2Loader chunk base64)
- [x] 5.2 从 `packages/react/vite.config.ts` 与 `packages/vue/vite.config.ts` 的 external 列表移除这两项
...
- [ ] 7.1 在临时目录用 `pnpm create vite@latest` 新建一个 React + Vite + pnpm 严格模式项目,`pnpm add file:packages/react/...tgz`(用 `pnpm pack` 生成的 tarball),验证预览 `.woff2` / `.msg` 文件零配置工作 — **跳过(需用户授权后再做)**
- [ ] 7.2 同 7.1,对 Vue 包做一遍 — **跳过(需用户授权后再做)**

## 8. 收尾

- [x] 8.1 检查 `packages/example/vite.config.ts` 与 `packages/vue-example/vite.config.ts`,确认 `resolve.alias` 仅剩 `@opsbli/*` 入口 alias ✓
