...
- 响应式处理不统一（有的用 `md:pio-text-xl`，有的固定 `pio-text-lg`）

**约束：**
- 保持 Design System 一致性（Tailwind CSS + `pio-` 前缀）
- 不改变现有渲染器的 API 和行为
- 支持 i18n（错误信息通过参数传入）
- React 和 Vue 需要独立实现
...
**决策：**
- React: `packages/react/src/renderers/RendererError.tsx`
- Vue: `packages/vue/src/renderers/RendererError.vue`
