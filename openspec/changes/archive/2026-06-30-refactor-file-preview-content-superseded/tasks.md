...
- [x] 1.1 备份当前 `PioneerContent.tsx` 文件（重命名为 `PioneerContent.tsx.backup`）
...
- [x] 2.3 创建 `hooks/usePioneerState.ts`，封装 reducer 和自动重置逻辑
...
- [x] 5.2 创建 `components/preview/PioneerToolbar.tsx`，负责工具栏渲染
...
- [x] 5.5 创建 `components/preview/PioneerRenderer.tsx`，渲染器容器（包含 Suspense + 错误边界）
- [x] 5.6 将 `NavArrows` 提取到独立文件 `components/preview/NavArrows.tsx`
- [x] 5.7 创建 `components/preview/index.ts`，添加 barrel exports

## 6. 主组件重构（Phase 3）

- [x] 6.1 重构 `PioneerContentInner`，使用新的 hooks（usePioneerState、useKeyboardNavigation 等）
- [x] 6.2 替换分散的 useState 为 useReducer + dispatch
- [x] 6.3 使用 `useToolbarConfig` 替换 80 行 if-else 工具栏配置
- [x] 6.4 使用 `PioneerToolbar` 子组件替换内联工具栏 JSX
- [x] 6.5 使用 `PioneerRenderer` 子组件替换内联渲染器 JSX
...
- [x] 7.1 为内容容器添加 `data-scroll-container` 属性，替换 `.pio-overflow-auto` 选择器
...
- [ ] 14.2 删除 `PioneerContent.tsx.backup` 备份文件（建议在验证完成后再删除）
