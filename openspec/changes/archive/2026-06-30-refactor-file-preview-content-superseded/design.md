...
当前 `PioneerContent.tsx` 是文件预览组件的核心，负责渲染 10+ 种文件类型（图片、PDF、DOCX、视频、音频、EPUB、Mobi、ZIP 等）。组件包含：
...
```
components/
├── PioneerToolbar.tsx    # 工具栏 UI
├── PioneerRenderer.tsx   # 渲染器容器（Suspense + 错误边界）
└── NavArrows.tsx              # 导航箭头（已独立，保持不变）

hooks/
├── usePioneerState.ts    # 状态管理（reducer + actions）
...
**理由**：
- 避免硬编码 CSS 类名（如 `.pio-overflow-auto`），解耦样式和逻辑
...
```typescript
// Before
const container = contentRef.current?.querySelector('.pio-overflow-auto');
...
1. 创建 `hooks/usePioneerState.ts`，实现 reducer 和 actions
2. 创建 `hooks/useBookRenderer.ts`，统一 EPUB/Mobi 逻辑
3. 创建 `hooks/useKeyboardNavigation.ts`，提取键盘事件处理
4. 在 `PioneerContentInner` 中逐步替换 useState 为 useReducer
5. 运行测试，确保功能无回归
...
### Phase 3: 拆分子组件（1-2 天）
1. 创建 `components/PioneerToolbar.tsx`
2. 创建 `components/PioneerRenderer.tsx`（包含错误边界）
3. 重构 `PioneerContentInner`，使用新子组件
...
### Rollback Strategy
- 每个 Phase 独立提交，可单独回滚
- 保留原始 `PioneerContent.tsx.backup` 直到重构完全验证
...
1. **是否需要为新 hooks 添加单元测试？**
   - 倾向：是，特别是 `usePioneerState` reducer 逻辑
   - 影响：增加 1-2 天开发时间

2. **是否需要支持 Vue 版本的同步重构？**
   - 当前：仅重构 React 版本
   - 考虑：Vue 版本 (`vue`) 是否有相同问题？
