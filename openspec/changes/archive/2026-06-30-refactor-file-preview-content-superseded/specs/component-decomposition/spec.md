...
### Requirement: PioneerToolbar component
系统必须提供 PioneerToolbar 组件，负责渲染顶部工具栏 UI。
...
### Requirement: PioneerRenderer component
系统必须提供 PioneerRenderer 组件，负责渲染器容器和错误处理。
...
- **THEN** 仅 NavArrows 组件内部状态更新，不触发 PioneerContent 重渲染
...
#### Scenario: PioneerContent props unchanged
- **WHEN** 外部使用 PioneerContent 组件
- **THEN** 所有现有 props（files、currentIndex、onNavigate 等）保持不变

#### Scenario: Internal component props
- **WHEN** 内部使用 PioneerToolbar 或 PioneerRenderer
