...
- [x] 1.1 在 `packages/react/src/renderers/base.types.ts` 中扩展 `RendererHandle` 接口，添加可选的 `onToolbarChange` 方法
...
- [x] 3.1 修改 `PioneerContent.tsx` 中的 `useEffect`，检测 `rendererRef.current?.onToolbarChange` 是否存在
...
- [x] 8.4 删除 `usePioneerState` reducer 及其状态管理
...
- [x] 8.11 删除文件：rendererReducer.ts、usePioneerState.ts、useToolbarConfig.ts
