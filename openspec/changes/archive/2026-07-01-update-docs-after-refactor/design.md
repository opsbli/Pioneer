...
3. **i18n 体系**：字典权威源在 `core/src/i18n/messages/`，React 和 Vue 通过 `useTranslator` 消费，禁止硬编码文案
4. **主题系统**：通过 CSS 变量定义语义化 token（`--pio-fg-primary` 等），Tailwind 暴露为类（`text-fg-primary` 等），支持 `'auto' | 'dark' | 'light'` 三种模式
