...
`FontRenderer`（[packages/react/src/renderers/Font/index.tsx](packages/react/src/renderers/Font/index.tsx) 与 [packages/vue/src/renderers/Font/index.vue](packages/vue/src/renderers/Font/index.vue)）用于预览 TTF / OTF / WOFF / WOFF2 字体。原实现对 WOFF2 走 `wawoff2 → decompress(Brotli) → opentype.parse` 的链路，目标是在 FontFace 原生渲染之外再额外提供 family / glyphCount / version / designer 等元数据。
...
3. 构建验证：`pnpm --filter @pioneer/react build` 与 `pnpm --filter @pioneer/vue build`，确认主 bundle 与 Font chunk 中 `wawoff2` / `decompress_binding` / `onRuntimeInitialized` 命中数为 0。
