...
- [x] 1.1 在 `packages/core/src/types.ts` 中（或新建 `custom-renderer.ts`）导出框架无关的 `CustomRendererEventPayload` 类型：`{ name: string; payload?: unknown; file: PreviewFile }`
...
- [x] 2.1 在 `packages/react/src/types.ts` 新增 `CustomRendererContext`：包含 `emit(name, payload?)`、`t: Translator`、`theme: 'dark' | 'light'`、`locale: Locale`
...
- [x] 2.3 从 `@pioneer/core` re-export `CustomRendererEventPayload` 并在 `src/index.ts` 暴露

## 3. React 端 PioneerContent 接入

- [x] 3.1 `PioneerContentProps` 新增 `onCustomEvent?: (e: CustomRendererEventPayload) => void`
...
- [x] 4.1 `PioneerModal` props 新增 `onCustomEvent`，透传到 `PioneerContent`
- [x] 4.2 `PioneerEmbed` props 新增 `onCustomEvent`，透传到 `PioneerContent`

## 5. Vue 端类型与 ctx 扩展

- [x] 5.1 在 `packages/vue/src/types.ts` 新增 `CustomRendererContext`：字段与 React 端保持一致（`t` 的类型使用 core 导出的 `Translator`）
...
- [x] 6.5 `<component :is="customRendererComponent" :file="currentFile" :ctx="customCtx" />`；同时 `provide('pioneer:custom-ctx', customCtx)`，供深层子组件 `inject`
...
- [x] 7.1 `PioneerModal.vue` 声明并转发 `custom-event`
- [x] 7.2 `PioneerEmbed.vue` 声明并转发 `custom-event`
...
- [x] 9.1 在仓库根执行 `pnpm -r build`（或项目既定构建命令）确认 `core` / `react` / `vue` 全部通过
...
- [x] 10.2 在外层 `PioneerModal` 绑定事件出口，控制台打印收到的事件载荷
