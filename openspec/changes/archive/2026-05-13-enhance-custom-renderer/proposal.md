...
- 自定义渲染器的 `render` 签名扩展为 `render(file, ctx)`，`ctx` 中提供 `emit(eventName, payload)` 与 `t`（翻译器）、`theme` 等上下文；`emit` 会统一转发到 `PioneerContent` 的 `onCustomEvent` / Vue `custom-event` 事件
- `PioneerContent` 新增 prop `onCustomEvent`（React）/ `custom-event` emit（Vue），宿主应用可据此绑定自定义渲染器的事件
...
- 更新 `packages/react` 与 `packages/vue` 两端实现，保持 API 同构
...
- 代码：
  - `packages/react/src/types.ts`：扩展 `CustomRenderer`
  - `packages/react/src/PioneerContent.tsx`：接入自定义工具组与事件转发，新增 `onCustomEvent` prop
  - `packages/react/src/PioneerModal.tsx` / `PioneerEmbed.tsx`：透传新 prop
  - `packages/vue/src/types.ts`：扩展 `CustomRenderer`
  - `packages/vue/src/PioneerContent.vue`：接入自定义工具组与事件转发，新增 `custom-event` emit
  - `packages/vue/src/PioneerModal.vue` / `PioneerEmbed.vue`：透传事件
- API：`CustomRenderer` 接口向后兼容扩展；`PioneerContent` / `PioneerModal` / `PioneerEmbed` 新增一个可选事件入口
