...
#### Scenario: 提供懒加载注册示例
- **WHEN** 文档展示懒加载流程
- **THEN** 包含 `lazy.ts` 注册代码和 `PioneerContent.vue` 导入示例
...
- 语义化 token 系统（`pio-text-fg-primary`、`pio-bg-surface-1` 等）
- `useResolvedTheme()` composable 的使用
- `<style scoped>` 中使用 `var(--pio-*)` 变量
- 禁止字面色类和硬编码颜色值的约束

#### Scenario: 开发者使用语义化 token
- **WHEN** 自定义 renderer 需要设置颜色
- **THEN** 能在 template 中使用 `pio-text-fg-*` / `pio-bg-surface-*` 等语义类，在 style 中使用 `var(--pio-*)` 变量
