...
- **THEN** it can import and use `RendererError` from `@pioneer/react`

#### Scenario: Vue component
- **WHEN** a Vue renderer needs to display an error
- **THEN** it can import and use `RendererError` from `@pioneer/vue`
...
The `RendererError` (and any sibling shared components such as `RendererLoading`) SHALL use the Tailwind class prefix declared by the package it ships in. The React package uses `pio-` and the Vue package uses `pio-`; cross-package copy-paste MUST update every class accordingly.

#### Scenario: React package classes
- **WHEN** a shared renderer component lives in `packages/react`
- **THEN** every Tailwind utility class in its template/JSX SHALL be prefixed with `pio-` (e.g. `pio-flex`, `pio-text-lg`, `md:pio-text-xl`)

#### Scenario: Vue package classes
- **WHEN** a shared renderer component lives in `packages/vue`
- **THEN** every Tailwind utility class in its template SHALL be prefixed with `pio-` (e.g. `pio-flex`, `pio-text-lg`, `md:pio-text-xl`)
...
- **THEN** the prefix appears between the variant and the utility name (e.g. `md:pio-text-xl`, not `pio-md:text-xl`)
