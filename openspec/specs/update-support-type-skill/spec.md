...
- PioneerContent 接入"五件事"（Import、Ref + State、Handler、Toolbar 分支、渲染分支）
...
- `PioneerContent` 导入约束（从 `./renderers/lazy` 导入，禁止直接 import renderer 本体）
...
- 字典权威源（`core/src/i18n/messages/zh-CN.ts` 和 `en-US.ts`）
...
- **THEN** 明确 `@pioneer/core` 绝不能加进 external 列表
