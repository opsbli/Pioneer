...
### Requirement: PioneerModal SHALL accept showDownload prop

`PioneerModal` 组件(React 和 Vue 版本)SHALL 接受 `showDownload` prop 来控制下载按钮的显示。
...
### Requirement: PioneerEmbed SHALL accept showDownload prop

`PioneerEmbed` 组件(React 和 Vue 版本)SHALL 接受 `showDownload` prop 来控制下载按钮的显示。
...
### Requirement: PioneerContent SHALL propagate showDownload prop

`PioneerContent` 组件(React 和 Vue 版本)SHALL 接受 `showDownload` prop 并将其传递给工具栏组件。

#### Scenario: 接收并透传 showDownload prop
- **WHEN** `PioneerContent` 接收到 `showDownload` prop
- **THEN** 该 prop SHALL 被传递给内部的工具栏渲染逻辑
- **AND** 不对该 prop 进行任何转换或修改

#### Scenario: 默认值透传
- **WHEN** `PioneerContent` 未接收到 `showDownload` prop
- **THEN** 组件 SHALL 使用默认值 `true`
- **AND** 将该默认值传递给工具栏渲染逻辑

### Requirement: React PioneerToolbar SHALL conditionally render download button

React 版本的 `PioneerToolbar` 组件 SHALL 根据 `showDownload` prop 条件渲染下载按钮。

#### Scenario: showDownload 为 true 时渲染下载按钮
- **WHEN** `PioneerToolbar` 接收到 `showDownload={true}`
- **THEN** 下载按钮 SHALL 包含在 `actionGroups` 数组中
- **AND** 下载按钮 SHALL 在桌面端和移动端工具栏中都正常显示

#### Scenario: showDownload 为 false 时不渲染下载按钮
- **WHEN** `PioneerToolbar` 接收到 `showDownload={false}`
- **THEN** 下载按钮 SHALL NOT 包含在 `actionGroups` 数组中
- **AND** 关闭按钮(如果存在)SHALL 保持正常显示
- **AND** 渲染器工具栏项(toolGroups)SHALL 不受影响

### Requirement: Vue PioneerContent SHALL conditionally render download button

Vue 版本的 `PioneerContent` 组件 SHALL 根据 `showDownload` prop 条件渲染下载按钮。

#### Scenario: showDownload 为 true 时渲染下载按钮
- **WHEN** `PioneerContent` 接收到 `:show-download="true"`
- **THEN** 下载按钮 SHALL 包含在 `actionGroups` 计算属性中
- **AND** 下载按钮 SHALL 在桌面端和移动端工具栏中都正常显示

#### Scenario: showDownload 为 false 时不渲染下载按钮
- **WHEN** `PioneerContent` 接收到 `:show-download="false"`
