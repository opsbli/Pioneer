...
```bash
git clone https://github.com/YOUR_USERNAME/Pioneer.git
cd Pioneer
```
...
```bash
git remote add upstream https://github.com/opsbli/Pioneer.git
```
...
- **Components**: PascalCase (e.g., `PioneerModal`)
...
refactor(vue): simplify PioneerEmbed props
...
1. **Add type detection** in `packages/core/src/utils/mimeTypes.ts`
2. **Create renderer component** in `packages/react/src/renderers/` (and Vue equivalent)
3. **Add loader logic** in `packages/core/src/loaders/` if needed
...
- Open an [issue](https://github.com/opsbli/Pioneer/issues) for bug reports or feature requests
- Join discussions in [GitHub Discussions](https://github.com/opsbli/Pioneer/discussions)
