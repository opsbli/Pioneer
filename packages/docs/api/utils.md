...
构造一个统一的 `fetcher`，库内 / 用户代码均可用它替代裸 `fetch` 以复用鉴权配置。组件层通常无需直接调用——`PioneerModal` / `Embed` / `Content` 的 `requestInit` / `requestHandler` props 会在内部调它。仅在你自己实现 `customRenderer` 且需要拉取远程资源时才需要它。
...
```typescript
import { createFetcher, fetchAsBlobUrl } from '@pioneer/core'
...
```typescript
import { createFetcher, fetchTextUtf8 } from '@pioneer/core'
