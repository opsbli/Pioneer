...
- **`PioneerModal`** — 全屏弹窗预览,默认通过 Portal/Teleport 挂载到 `document.body`
- **`PioneerEmbed`** — 嵌入式预览,直接内联到你指定的 div 容器中
...
`PioneerModal` 使用 React Portal（或 Vue Teleport）将模态框渲染到 `document.body`，这确保了：
...
这意味着你可以在任何位置使用 `PioneerModal`，无需担心层级和定位问题。
