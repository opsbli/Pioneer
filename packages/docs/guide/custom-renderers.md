...
- 通过 `ctx.emit(name, payload)` **派发事件**，宿主在 `PioneerContent` / `Modal` / `Embed` 上通过 `onCustomEvent`（React）或 `@custom-event`（Vue）统一接收
...
- `ctx.emit(name, payload)` 会通过顶层 `PioneerContent` 的 `onCustomEvent` / `custom-event` 出口向宿主转发
- 载荷固定为 `{ name, payload, file }`（`file` 为当前渲染的 `PreviewFile`）
- 宿主未绑定监听时静默忽略，不抛错
- `PioneerModal` / `PioneerEmbed` 会自动透传事件
...
**字典位置：** `core/src/i18n/messages/zh-CN.ts` 和 `en-US.ts`
...
export const CustomRenderer = forwardRef<RendererHandle, Props>((props, ref) => {
  return (
    <div className="pio-bg-surface-1 pio-text-fg-primary pio-border pio-border-line">
      <button className="pio-bg-surface-2 hover:pio-bg-surface-3">
        {props.label}
      </button>
    </div>
  );
...
```vue
<template>
  <div class="pio-bg-surface-1 pio-text-fg-primary pio-border pio-border-line">
    <button class="pio-bg-surface-2 hover:pio-bg-surface-3">
      {{ label }}
    </button>
  </div>
</template>
