...
// 跟随系统
<PioneerModal theme="auto" files={files} ... />

// 嵌入模式同样支持
<PioneerEmbed theme="light" files={files} />
```
...
```tsx [React]
<PioneerEmbed headless files={files} />
```
...
```css
/* 自定义 light 主题的滚动条 */
.pio-root[data-theme="light"] ::-webkit-scrollbar-thumb {
  background: rgba(0, 0, 0, 0.3);
}

/* 自定义 dark 主题的滚动条 */
.pio-root[data-theme="dark"] ::-webkit-scrollbar-thumb {
...
组件内部使用带前缀的 Tailwind 类（`pio-` / `pio-`），不会与你项目中的 Tailwind 冲突。如需进一步定制，可通过 `data-theme` 属性选择器配合你自己的 Tailwind 类。
