...
Pioneer 支持多种常见文件格式，React 与 Vue 版本共享类型识别和解析核心，并提供等价的预览能力。
...
- **前端框架组件** - `.vue`, `.svelte`（shiki 高亮 template / script / style 分区）
- **YAML** - `.yaml`, `.yml`
- **TOML** - `.toml`
- **INI** - `.ini`, `.conf`, `.env`
- **Lock 文件** - `.lock`
...
- **Shell** - `.sh`, `.bash`, `.zsh`
- **Batch** - `.bat`（Windows 批处理脚本，shiki 高亮）
- **SQL** - `.sql`
- **Log** - `.log`
...
- 基于 @zip.js/zip.js 解析压缩包目录结构
- **支持加密 ZIP**（传统 ZipCrypto / AES，自动弹出密码输入框，错误密码可重试 3 次）
- 左侧树形目录 + 右侧内嵌预览
- 内嵌预览文本、代码（带高亮）与图片
