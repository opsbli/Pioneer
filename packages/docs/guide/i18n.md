...
React Pioneer 内置极简的国际化方案，支持 **中文（zh-CN，默认）** 和 **英文（en-US）** 两种语言。
...
`locale` prop 同时适用于 `PioneerModal`、`PioneerEmbed`、`PioneerContent` 三个组件。
...
<PioneerModal files={files} locale={locale} />
```
...
详见 `packages/core/src/i18n/messages/zh-CN.ts` 和 `en-US.ts` 的完整字典文件。
