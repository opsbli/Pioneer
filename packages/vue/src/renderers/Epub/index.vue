...
<template>
  <div class="pio-relative pio-w-full pio-h-full pio-flex pio-justify-center pio-bg-surface-1 pio-overflow-hidden">
    ...
    <div
      v-if="toc.length > 0"
      class="pio-absolute pio-inset-0 pio-z-20 pio-flex"
      :style="{ opacity: showToc ? 1 : 0, pointerEvents: showToc ? 'auto' : 'none', transition: 'opacity 0.3s' }"
    >
      <div
        class="pio-w-72 pio-h-full pio-flex pio-flex-col"
        :style="{
          maxWidth: '80%',
      ...
      >
        <div class="pio-flex pio-items-center pio-justify-between pio-flex-shrink-0" style="padding: 12px 16px; border-bottom: 1px solid rgba(255,255,255,0.1)">
        ...
        <div class="pio-flex-1 pio-overflow-y-auto" style="padding: 8px 4px">
    ...
    <div
      v-if="!error"
      ref="viewerRef"
      class="pio-h-full pio-bg-surface-toolbar"
      :style="{
