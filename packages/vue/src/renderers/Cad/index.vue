...
<template>
  <div ref="containerRef" class="pio-relative pio-w-full pio-h-full pio-bg-media-bg">
    <!-- 错误覆盖层 -->
    <div
      v-if="error"
      class="pio-absolute pio-inset-0 pio-flex pio-items-center pio-justify-center pio-bg-surface-3 pio-z-10"
    >
      <div class="pio-text-center">
        <p class="pio-text-fg-primary">{{ error }}</p>
      </div>
    </div>
    ...
    >
      <div class="pio-text-center">
        <div class="pio-inline-block pio-w-8 pio-h-8 pio-border-4 pio-border-spinner-track pio-border-t-spinner-head pio-rounded-full pio-animate-spin" />
        <p class="pio-mt-4 pio-text-fg-secondary">{{ t('cad.loading') }}</p>
      </div>
    </div>
  </div>
</template>
