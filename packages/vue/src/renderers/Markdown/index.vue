...
const highlightAndInjectCopyButtons = async () => {
  ...
  for (const pre of pending) {
    ...
    try {
      ...
      pre.style.backgroundColor = 'var(--pio-code-bg)';
...
<template>
  ...
  <!-- 源码视图 -->
  <div v-else-if="viewMode === 'source'" class="pio-w-full pio-h-full pio-overflow-auto" style="background: var(--pio-code-bg);">
    <pre
      v-if="!highlightedSource"
      class="pio-p-6 pio-text-fg-primary pio-font-mono pio-text-sm pio-whitespace-pre-wrap pio-break-words"
      >{{ content }}</pre
    >
  ...
  <!-- 预览视图 -->
  <div v-else class="pio-w-full pio-h-full pio-overflow-auto pio-py-6 pio-px-4">
...
.markdown-body :deep(.table-wrapper) {
  ...
  border: 1px solid var(--pio-surface-3);
}
