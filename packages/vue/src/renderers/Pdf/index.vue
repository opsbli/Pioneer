  ...
  return items.map((item, i) => {
    ...
    let html = `<li style="margin-left: ${depth > 0 ? 16 : 0}px;">
      <button
        ...
        class="pio-w-full pio-text-left pio-py-2 pio-px-3 pio-text-sm pio-rounded pio-transition-all pio-truncate ${activeClass}"
        title="${item.title}"
      >
        ${item.title}
      </button>`;
...
const loadPdf = async (opts?: { resetRetries?: boolean }) => {
  ...
  passwordError.value = '';
  if (resetRetries) {
    passwordRetries.value = MAX_PASSWORD_RETRIES;
  }
  ...
  try {
    ...
    if (props.password || currentPassword.value) {
      loadOptions.password = props.password || currentPassword.value;
    }
    ...
    if (isPasswordError) {
      const hasPassword = !!props.password || !!currentPassword.value;
      if (hasPassword) {
        // 已提供密码但错误
        const remaining = passwordRetries.value - 1;
        passwordRetries.value = remaining;
        if (remaining <= 0) {
...
// 监听 password prop 变化，重新加载
watch(() => props.password, () => {
  if (props.password && needsPassword.value) {
...
<template>
  <div ref="containerRef" class="pio-relative pio-w-full pio-h-full">
    <!-- 大纲侧边栏 -->
    <div
      v-if="outline.length > 0"
      class="pio-absolute pio-inset-0 pio-z-20 pio-flex pio-transition-opacity pio-duration-300"
      :style="{
    ...
    >
      <div
        class="pio-w-72 pio-max-w-[80%] pio-h-full pio-bg-surface-overlay pio-backdrop-blur-xl pio-border-r pio-border-line-weak pio-flex pio-flex-col pio-shadow-2xl pio-transition-transform pio-duration-300"
        :style="{ transform: showOutline ? 'translateX(0)' : 'translateX(-100%)' }"
      >
        <div class="pio-flex pio-items-center pio-justify-between pio-px-4 pio-py-3 pio-border-b pio-border-line-weak pio-flex-shrink-0">
          <span class="pio-text-fg-primary pio-font-medium pio-text-sm">{{ t('toolbar.outline') }}</span>
          <button
            @click="showOutline = false"
            class="pio-text-fg-tertiary hover:pio-text-fg-primary pio-transition-colors"
          >
            <X class="pio-w-4 pio-h-4" />
          </button>
        </div>
        <div class="pio-flex-1 pio-overflow-y-auto pio-py-4 pio-px-1 outline-items" v-html="renderOutlineItemsHtml(outline)" />
      </div>
      <div
        class="pio-flex-1 pio-transition-opacity pio-duration-300"
    ...
    <div
      ...
      class="pio-pdf-container pio-w-full pio-h-full pio-overflow-auto"
    >
      <RendererError v-if="error" :message="error" />

      <div v-if="!error && isLoading" class="pio-flex pio-items-center pio-justify-center pio-min-h-screen">
        <div
          class="pio-w-12 pio-h-12 pio-border-4 pio-border-line-strong pio-border-t-spinner-head pio-rounded-full pio-animate-spin"
        />
      </div>

      <div v-if="!error" class="pio-flex pio-flex-col pio-items-center">
        <div class="pdf-pages pio-flex pio-flex-col pio-gap-4" />
      </div>
    </div>

    <!-- 密码弹窗等待态 -->
    <div
      v-if="needsPassword && !error"
      class="pio-absolute pio-inset-0 pio-flex pio-items-center pio-justify-center pio-z-[5] pio-bg-black/20"
    >
