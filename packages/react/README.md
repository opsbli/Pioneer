...
Overlay a watermark on the preview content via the `watermark` prop (`pointer-events: none`, does not interfere with interactions). Colors adapt to the theme by default (white in dark / black in light); opacity is controlled by `opacity`.
...
<PioneerModal
  ...
  isOpen={isOpen}
  onClose={() => setIsOpen(false)}
  // Text watermark (tiled by default)
  watermark={{ mode: 'text', text: 'Confidential', rotation: -30 }}
  // Image watermark + centered
  // watermark={{ mode: 'image', imageUrl: '/logo.png', position: 'center', opacity: 0.3 }}
/>
```

`layers` stacks multiple text/image watermark layers — array order is the draw order (later layers on top), and each layer supports independent `text`/`imageUrl`, `font`, `color`, `imageSize`, `opacity`, `rotation`, `spacing` and `position`. When `layers` is provided, the top-level single-layer fields are ignored. Example: logo image + username/timestamp text:
...
<PioneerModal
  ...
  watermark={{
    mode: 'text',
    layers: [
      { type: 'image', imageUrl: '/logo.png', position: 'center', opacity: 0.3 },
      { type: 'text', text: '{username} {time}', position: 'tile', rotation: -30 },
    ],
  }}
/>
```
...
- **Frontend Frameworks**: Vue (.vue), Svelte (.svelte)
- **Scripts**: Shell (.sh/.bash/.zsh), Batch (.bat)
...
| `watermark` | `WatermarkConfig` | ❌ | Watermark config (text/image, tile/center/diagonal) |
| `password` | `string` | ❌ | Password for encrypted PDF/ZIP (password dialog auto-opens when omitted) |
...
> `PioneerEmbed` has no `isOpen` / `onClose`. To hide/show it, conditionally render it from the parent. The close button is hidden by default but can be enabled via `showClose`.

### PioneerContent (advanced)
...
```tsx
// Switch to English
<PioneerModal files={files} locale="en-US" ... />
...
- **Dictionary Source**: `core/src/i18n/messages/` (zh-CN.ts, en-US.ts)
- **No Hardcoding**: All user-visible text must use translation keys
- **Automatic Locale Switching**: Follows the `locale` prop passed to `PioneerModal`
...
export const CustomRenderer = forwardRef<RendererHandle, Props>((props, ref) => {
  ...
  if (error) {
    return (
      <div className="pio-text-fg-primary">
...
All colors are defined as CSS variables (`--pio-*`) and exposed via Tailwind classes with the `pio-` prefix:

| Usage | Class | Description |
|-------|-------|-------------|
| **Text (fg)** | | |
| Primary text | `pio-text-fg-primary` | Highest contrast |
| Body text | `pio-text-fg-secondary` | Default text |
| Secondary text | `pio-text-fg-tertiary` | Captions, counters |
| Muted text | `pio-text-fg-muted` | Placeholders |
| Disabled text | `pio-text-fg-disabled` | Disabled buttons |
| **Background (surface)** | | |
| Surface 1 | `pio-bg-surface-1` | Cards, weakest |
| Surface 2 | `pio-bg-surface-2` | Hover states |
| Surface 3 | `pio-bg-surface-3` | Emphasis |
| Toolbar | `pio-bg-surface-toolbar` | Top toolbar |
| **Borders** | | |
| Weak border | `pio-border-line-weak` | Subtle lines |
| Standard border | `pio-border-line` | Default borders |
| Strong border | `pio-border-line-strong` | Emphasis |
| **Code** | | |
| Code background | `pio-bg-code-bg` | Dark: #1e1e1e / Light: #f6f8fa |
| Code text | `pio-text-code-fg` | Follows theme |
| **Accent** | | |
| Accent background | `pio-bg-accent` | Primary buttons |
| Accent hover | `pio-bg-accent-hover` | Hover state |
...
export const CustomRenderer = forwardRef<RendererHandle, Props>((props, ref) => {
  return (
    <div className="pio-bg-surface-1 pio-border pio-border-line-weak pio-rounded">
      <h2 className="pio-text-fg-primary pio-text-lg">Title</h2>
      <p className="pio-text-fg-secondary">Body text</p>
      <button className="pio-bg-surface-2 hover:pio-bg-surface-3 pio-text-fg-primary">
        Click me
      </button>
      <pre className="pio-bg-code-bg pio-text-code-fg">
        {code}
      </pre>
    </div>
  );
...
<div className="pio-text-gray-700 pio-bg-gray-100">

// ❌ Inline literal colors
<div style={{ color: '#ffffff', background: '#1f2937' }}>

// ❌ Hardcoded dark-only colors
<div style={{ background: '#1e1e1e' }}>  // Use pio-bg-code-bg instead
```
...
- `ESC` - Close preview (closes search panel first if open)
- `←` / `→` - Previous/next file (not triggered while search panel is open)
- `Ctrl+F` / `Cmd+F` - Open full-text search panel
- `↑` / `↓` - While search panel is open: jump to previous/next match
...
- [Online Demo](https://opsbli.github.io/Pioneer) - Live demo
...
```bash
# Clone repository
git clone https://github.com/opsbli/Pioneer.git
...
│   ├── PioneerModal.tsx  # Main component
