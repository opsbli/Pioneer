...
export const ResizableSplit = forwardRef<ResizableSplitHandle, ResizableSplitProps>(({
  ...
  if (mobileTabMode && !isDesktop) {
    return (
      <div
        ref={containerRef}
        className={`pio-w-full pio-h-full pio-flex pio-flex-col pio-min-h-0 pio-min-w-0 ${className}`}
      >
        <div className="pio-flex pio-flex-shrink-0 pio-border-b pio-border-line-weak pio-bg-surface-toolbar">
          <button
            type="button"
            onClick={() => setActiveTab('left')}
            className={`pio-flex-1 pio-py-2.5 pio-text-sm pio-transition-colors ${
              activeTab === 'left'
        ...
        <div
          className="pio-flex-1 pio-min-h-0 pio-min-w-0 pio-w-full pio-overflow-hidden"
  ...
  return (
    <div
      ref={containerRef}
      className={`pio-w-full pio-h-full pio-flex pio-flex-col md:pio-flex-row pio-min-h-0 pio-min-w-0 ${className}`}
    >
      <div
        className="pio-min-h-0 pio-min-w-0 pio-flex-shrink-0 pio-w-full pio-max-h-60 md:pio-h-full md:pio-max-h-none"
      ...
      <div
        ...
        className={`pio-hidden md:pio-block pio-relative pio-w-1.5 pio-flex-shrink-0 pio-cursor-col-resize pio-transition-colors ${
          dragging ? 'pio-bg-surface-toolbar' : 'pio-bg-surface-2 hover:pio-bg-surface-3'
        }`}
      >
        {/* 加宽命中区，改善拖动体验 */}
        <span className="pio-absolute pio-inset-y-0 -pio-left-1 -pio-right-1" />
      </div>
      <div className="pio-flex-1 pio-min-w-0 pio-min-h-0 pio-overflow-hidden">{right}</div>
    </div>
  );
});
