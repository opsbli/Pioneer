    ...
    <React.Fragment key={gi}>
      {group.items.map((item, ii) =>
        item.type === 'button' ? (
          ...
          <span
            key={`${gi}-${ii}`}
            className="pio-text-xs pio-text-center pio-font-medium pio-tabular-nums pio-text-fg-tertiary"
      ...
      {gi < arr.length - 1 && <div className={`pio-w-px pio-h-4 pio-bg-divide ${dividerClass}`} />}
    </React.Fragment>
  ));
}
