...
export const FontRenderer = forwardRef<RendererHandle, FontRendererProps>(({ url }, ref) => {
  ...
  if (loading) {
    return (
      <div className="pio-flex pio-items-center pio-justify-center pio-w-full pio-h-full">
        <div className="pio-text-fg-secondary">{t('font.loading')}</div>
      </div>
    );
  }
  ...
  return (
    <div className="pio-flex pio-flex-col pio-w-full pio-h-full pio-overflow-hidden pio-min-w-0">
      {/* 元数据区 */}
      <div className="pio-flex-shrink-0 pio-px-6 pio-py-4 pio-bg-surface-1 pio-border-b pio-border-line-weak pio-min-w-0">
        <div className="pio-grid pio-grid-cols-2 pio-gap-x-6 pio-gap-y-2 pio-text-sm pio-min-w-0">
          <div className="pio-flex pio-items-center pio-gap-2 pio-min-w-0">
            <span className="pio-text-fg-tertiary pio-flex-shrink-0">{t('font.meta.family')}:</span>
            <span className="pio-text-fg-primary pio-font-semibold pio-truncate">
              {showMetaPlaceholder ? metaPlaceholder : metadata.family}
            </span>
          </div>
          {metadata.subfamily && (
            <div className="pio-flex pio-items-center pio-gap-2 pio-min-w-0">
              <span className="pio-text-fg-tertiary pio-flex-shrink-0">{t('font.meta.subfamily')}:</span>
              <span className="pio-text-fg-primary pio-truncate">{metadata.subfamily}</span>
            </div>
          )}
          <div className="pio-flex pio-items-center pio-gap-2 pio-min-w-0">
            <span className="pio-text-fg-tertiary pio-flex-shrink-0">{t('font.meta.format')}:</span>
            <span className="pio-text-fg-primary pio-truncate">{metadata.format}</span>
          </div>
          <div className="pio-flex pio-items-center pio-gap-2 pio-min-w-0">
            <span className="pio-text-fg-tertiary pio-flex-shrink-0">{t('font.meta.glyphs')}:</span>
            <span className="pio-text-fg-primary pio-truncate">
              {showMetaPlaceholder ? metaPlaceholder : metadata.glyphCount}
            </span>
          </div>
          {metadata.designer && (
            <div className="pio-flex pio-items-center pio-gap-2 pio-min-w-0">
              <span className="pio-text-fg-tertiary pio-flex-shrink-0">{t('font.meta.designer')}:</span>
              <span className="pio-text-fg-primary pio-truncate">{metadata.designer}</span>
            </div>
          )}
          {metadata.version && (
            <div className="pio-flex pio-items-center pio-gap-2 pio-min-w-0">
              <span className="pio-text-fg-tertiary pio-flex-shrink-0">{t('font.meta.version')}:</span>
              <span className="pio-text-fg-primary pio-truncate">{metadata.version}</span>
            </div>
          )}
        </div>
      </div>
      ...
      {/* 预览区 */}
      <div className="pio-flex-1 pio-overflow-auto pio-min-w-0">
        <div className="pio-px-6 pio-py-6 pio-space-y-8 pio-min-w-0">
          {/* 自定义输入框 */}
          <div className="pio-min-w-0">
            <label className="pio-block pio-text-xs pio-text-fg-tertiary pio-mb-2">
              {t('font.sample_text_placeholder')}
            </label>
            <textarea
              className="pio-block pio-w-full pio-box-border pio-px-3 pio-py-2.5 pio-bg-surface-2 pio-text-fg-primary pio-text-sm pio-leading-relaxed pio-border pio-border-line pio-rounded-md pio-resize-y focus:pio-outline-none focus:pio-border-line-strong"
              rows={2}
          ...
          {/* 多字号展示 */}
          <div className="pio-space-y-6 pio-min-w-0">
            {SIZES.map((size) => (
              <div key={size} className="pio-space-y-2 pio-min-w-0">
                <div className="pio-text-xs pio-text-fg-tertiary">{size}px</div>
                <FontPreviewLine
                  font={font}
                  text={displayText}
                  fontSize={size}
          ...
          {/* 字符集样例 */}
          <div className="pio-space-y-6 pio-pt-6 pio-border-t pio-border-line-weak pio-min-w-0">
            <div className="pio-min-w-0">
              <div className="pio-text-sm pio-text-fg-tertiary pio-mb-3">Latin Alphabet</div>
              <FontPreviewLine
                font={font}
            ...
            <div className="pio-min-w-0">
              <div className="pio-text-sm pio-text-fg-tertiary pio-mb-3">Chinese Characters</div>
              <FontPreviewLine
                font={font}
...
const FontPreviewLine: React.FC<FontPreviewLineProps> = ({ font, text, fontSize, renderMode, theme }) => {
  ...
  if (renderMode === 'canvas') {
    return (
      <div ref={wrapperRef} className="pio-w-full">
        <canvas ref={canvasRef} className="pio-block" />
      </div>
    );
  }
  ...
  return (
    <div
      ref={wrapperRef}
      className="pio-w-full pio-text-fg-primary pio-whitespace-pre-wrap pio-break-words"
      style={{
