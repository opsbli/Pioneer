...
if (typeof document !== 'undefined' && !document.getElementById('pio-epub-styles')) {
  const styleEl = document.createElement('style');
  styleEl.id = 'pio-epub-styles';
  styleEl.textContent = `
...
export const EpubRenderer = forwardRef<EpubRendererHandle, EpubRendererProps>(
  ({ url }, ref) => {
    ...
    const getToolbarGroups = useCallback((): ToolbarGroup[] => [
      {
        items: [
          { type: 'button', icon: <List className="pio-w-4 pio-h-4" />, tooltip: t('toolbar.toc'), action: toggleToc, disabled: toc.length === 0, active: showToc },
        ],
      },
      {
        items: [
          { type: 'button', icon: <ChevronLeft className="pio-w-4 pio-h-4" />, tooltip: t('toolbar.prev_page'), action: handlePrev, disabled: currentChapter <= 1 },
          { type: 'text', content: `${currentChapter} / ${totalChapters}`, minWidth: '4rem' },
          { type: 'button', icon: <ChevronRight className="pio-w-4 pio-h-4" />, tooltip: t('toolbar.next_page'), action: handleNext, disabled: currentChapter >= totalChapters },
        ],
      },
      {
        items: [
          { type: 'button', icon: isFullWidth ? <Minimize2 className="pio-w-4 pio-h-4" /> : <Maximize2 className="pio-w-4 pio-h-4" />, tooltip: isFullWidth ? t('toolbar.normal_width') : t('toolbar.full_width'), action: toggleFullWidth, active: isFullWidth },
        ],
      },
          ...
          return (
            <li key={`${item.href}-${i}`}>
              <button
                ...
                className={`pio-w-full pio-text-left pio-py-2 pio-px-3 pio-text-sm pio-rounded pio-transition-all pio-truncate ${active
                    ? 'pio-text-fg-primary pio-bg-surface-3 pio-font-medium'
    ...
    return (
      <div className="pio-relative pio-w-full pio-h-full pio-flex pio-justify-center pio-bg-surface-1 pio-overflow-hidden">
        {error && <RendererError message={error} />}

        {loading && !error && (
          <div className="pio-absolute pio-inset-0 pio-flex pio-items-center pio-justify-center pio-z-10">
          ...
          >
            ...
            >
              <div className="pio-flex pio-items-center pio-justify-between pio-px-4 pio-py-3 pio-border-b pio-border-line-weak pio-flex-shrink-0">
                <span className="pio-text-fg-primary pio-font-medium pio-text-sm">{t('toolbar.toc')}</span>
                <button
        ...
        {!error && (
          <div
            ref={viewerRef}
            className="pio-h-full pio-bg-surface-toolbar pio-shadow-lg"
            style={{
