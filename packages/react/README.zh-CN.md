...
```tsx
import { PioneerModal } from 'react';
...
通过 `watermark` prop 在预览内容上叠加水印层（`pointer-events: none`，不干扰交互）。颜色默认按主题自适应（dark 白色 / light 黑色），透明度由 `opacity` 统一控制。
...
`WatermarkConfig` 字段：`mode`（`'text' | 'image'`，必填）、`layers`（多层叠加：`WatermarkLayer[]`，每层独立配置类型/内容/透明度/旋转/间距/布局，数组顺序即绘制顺序；传入时忽略顶层单层字段）、`text`、`font`、`color`、`imageUrl`、`imageSize`、`opacity`（默认 0.35）、`rotation`（默认 -30）、`spacing`（默认 [200, 150]）、`position`（`'tile' | 'center' | 'diagonal'`，默认 tile）、`zIndex`。

多层叠加：图片层 + 文字层任意混合（如"Logo 图片 + 用户名/时间戳文字"）：
...
加密 PDF 和加密 ZIP 自动弹出密码输入框（错误密码可重试 3 次）。已知密码时通过 `password` prop 直接传入，跳过弹窗。

```tsx
// 方式一：已知密码直接加载（加密 PDF / 加密 ZIP 通用）
<PioneerModal
  files={files}
  currentIndex={0}
  isOpen={isOpen}
...
// 方式二：不传密码，检测到加密文件自动弹窗输入
<PioneerModal files={files} currentIndex={0} isOpen={isOpen} onClose={close} />
```

> 支持加密 ZIP（传统 ZipCrypto / AES，基于 @zip.js/zip.js）。加密 Office 文件（docx/xlsx/pptx）暂不支持，会显示"文件受密码保护"提示。

### 全文搜索

文本类文件（代码/文本/JSON/XML/Markdown）支持全文搜索：

- **唤起**: 按 `Ctrl+F`（Mac 上 `Cmd+F`），无需额外配置
- **高亮**: 匹配文本黄色标记，当前匹配橙色高亮
- **导航**: 搜索面板内 `↑`/`↓` 或按钮跳转，`Esc` 关闭并清除高亮
...
- **前端框架**: Vue (.vue), Svelte (.svelte)
- **脚本**: Shell (.sh/.bash/.zsh), Batch (.bat)
...
| `watermark` | `WatermarkConfig` | ❌ | 水印配置（文字/图片、平铺/居中/对角线） |
| `password` | `string` | ❌ | 加密 PDF/ZIP 密码（不传时自动弹出密码输入框） |
...
> `PioneerEmbed` 没有 `isOpen` / `onClose`,若要显示/隐藏,请在父组件中条件渲染。关闭按钮默认隐藏，但可通过 `showClose` 启用。
...
- **字典源**：`core/src/i18n/messages/`（zh-CN.ts、en-US.ts）
- **禁止硬编码**：所有用户可见文案必须使用翻译 key
- **自动切换语言**：跟随 `PioneerModal` 的 `locale` prop
...
所有颜色定义为 CSS 变量（`--pio-*`），通过 Tailwind 类暴露，前缀为 `pio-`：

| 用途 | 类名 | 说明 |
|------|------|------|
| **文字（fg）** | | |
| 主文本 | `pio-text-fg-primary` | 最高对比度 |
| 正文 | `pio-text-fg-secondary` | 默认文字 |
| 次要文本 | `pio-text-fg-tertiary` | 副本、计数器 |
| 弱化文本 | `pio-text-fg-muted` | 占位符 |
| 禁用文本 | `pio-text-fg-disabled` | 禁用按钮 |
| **背景（surface）** | | |
| 表面层 1 | `pio-bg-surface-1` | 卡片、最弱 |
| 表面层 2 | `pio-bg-surface-2` | hover 状态 |
| 表面层 3 | `pio-bg-surface-3` | 强调 |
| 工具栏 | `pio-bg-surface-toolbar` | 顶部工具栏 |
| **边框** | | |
| 弱边框 | `pio-border-line-weak` | 细线 |
| 标准边框 | `pio-border-line` | 默认边框 |
| 强边框 | `pio-border-line-strong` | 强调 |
| **代码** | | |
| 代码背景 | `pio-bg-code-bg` | Dark：#1e1e1e / Light：#f6f8fa |
| 代码文字 | `pio-text-code-fg` | 跟随主题 |
| **强调（accent）** | | |
| 强调背景 | `pio-bg-accent` | 主按钮 |
| 强调 hover | `pio-bg-accent-hover` | hover 状态 |
...
export const CustomRenderer = forwardRef<RendererHandle, Props>((props, ref) => {
  return (
    <div className="pio-bg-surface-1 pio-border pio-border-line-weak pio-rounded">
      <h2 className="pio-text-fg-primary pio-text-lg">标题</h2>
      <p className="pio-text-fg-secondary">正文内容</p>
...
// ❌ 硬编码暗色
<div style={{ background: '#1e1e1e' }}>  // 应改用 pio-bg-code-bg
```
...
- `ESC` - 关闭预览（搜索面板打开时优先关闭搜索面板）
- `←` / `→` - 上一个/下一个文件（搜索面板打开时不触发）
...
- [在线演示](https://opsbli.github.io/Pioneer) - 在线 Demo
