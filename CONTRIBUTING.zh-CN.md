...
docs: 更新快速开始指南
refactor(vue): 简化 PioneerEmbed 组件 props
perf(react): 优化图片渲染性能
...
1. **添加类型检测** 在 `packages/core/src/utils/mimeTypes.ts`
2. **创建渲染器组件** 在 `packages/react/src/renderers/`（以及 Vue 对应版本）
3. **添加加载器逻辑** 在 `packages/core/src/loaders/`（如需要）
4. **更新文档** 在支持格式列表中
5. **添加示例** 到演示应用
...
- 提交 [Issue](https://github.com/opsbli/Pioneer/issues) 报告 bug 或请求新功能
- 在 [GitHub Discussions](https://github.com/opsbli/Pioneer/discussions) 参与讨论
